...
module.exports={
    hacerPedidoCountries:()=>{
        let cont=axios.get('https://restcountries.com/v3/all')
                ...
                .then(elemento=>{
                    ...
                    elemento.forEach(e=>{
                        objeto.push({ 
                            ...
                            capital:(e.capital&&Array.isArray(e.capital))?e.capital[0]:'not found',
                            continent:e.region,
    ...
    hacerPedidoPorNombre:(name)=>{
        let cont=axios.get(`https://restcountries.com/v3/name/${name}`)
                    .then(resul=>resul=resul.data)
                    .then(paises=>{
                        let objeto=[];
                        if(Array.isArray(paises)){paises.forEach(e=>{
                            objeto.push({
                                ID:e.cca3,
                                name:e.name.official,
                                capital:(e.capital&&Array.isArray(e.capital))?e.capital[0]:'not found',
